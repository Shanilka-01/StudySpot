...
if (!$place) {
    ...
    echo '<div class="container page"><h1>That study space does not exist</h1>
          <p class="muted">It may have been removed. <a href="' . url('pages/explore.php') . '">Back to Explore</a></p></div>';
...
<div class="container page">
    <p class="breadcrumb"><a href="<?= url('index.php') ?>">Home</a> &gt; <a href="<?= url('pages/explore.php') ?>">Explore</a> &gt; <?= e($place['name']) ?></p>
    ...
    <div class="detail">
        ...
        <aside class="card card__pad">
            ...
            <div style="display:flex;gap:12px;margin-top:18px;flex-wrap:wrap;">
                <?php if (is_logged_in()): ?>
                    <form method="post" action="<?= url('pages/toggle-favorite.php') ?>">
                        <input type="hidden" name="place_id" value="<?= $id ?>">
                        <input type="hidden" name="back" value="pages/place.php?id=<?= $id ?>">
                        <button class="btn btn--ghost" type="submit">
                            <?= $saved ? '&#10084; Saved' : '&#9825; Save' ?>
                        </button>
                    </form>
                <?php else: ?>
                    <a class="btn btn--ghost" href="<?= url('pages/login.php') ?>">&#9825; Save</a>
                <?php endif; ?>
                <a class="btn btn--ghost" href="<?= url('pages/map.php') ?>?id=<?= $id ?>">&#128506; Show on map</a>
            </div>

            <a class="btn btn--block" style="margin-top:16px;" href="<?= url('pages/booking.php') ?>?id=<?= $id ?>">Book your spot</a>
        </aside>
    </div>
    ...
    <div class="section-head">
        ...
        <a href="<?= url('pages/review.php') ?>?id=<?= $id ?>">Write a review</a>
    </div>

    <?php if (!$reviews): ?>
        <div class="card card__pad">
            <p>No reviews yet. <a href="<?= url('pages/review.php') ?>?id=<?= $id ?>" style="color:var(--green-700);font-weight:600;">Be the first to review this place.</a></p>
        </div>
    <?php endif; ?>
