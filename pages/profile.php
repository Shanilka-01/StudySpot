...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_profile') {
    ...
    if (!$errors) {
        ...
        if ($taken->fetchColumn()) {
            ...
            redirect('pages/profile.php?view=settings');
            exit;
        }
    }
}
...
<div class="profile">
    <aside class="side">
        ...
        <?php foreach ($menu as $key => [$label, $icon]): ?>
            <a href="<?= url('pages/profile.php') ?>?view=<?= $key ?>" class="<?= $view === $key ? 'is-active' : '' ?>">
                <span><?= $icon ?></span> <?= $label ?>
            </a>
        <?php endforeach; ?>
        <a href="<?= url('pages/my-bookings.php') ?>"><span>&#128197;</span> My Bookings</a>
        <a href="<?= url('pages/logout.php') ?>"><span>&#8631;</span> Logout</a>
    </aside>
    ...
    <section class="profile__body">
        ...
        <?php if ($view === 'overview' || $view === 'favorites'): ?>
            ...
            <?php if (!$favorites): ?>
                <p class="muted">Nothing saved yet. <a href="<?= url('pages/explore.php') ?>" style="color:var(--green-700);">Find a study space</a>.</p>
            <?php endif; ?>
        <?php endif; ?>
        ...
        <?php if ($view === 'overview' || $view === 'reviews'): ?>
            <div class="section-head">
                <h2>My Reviews</h2>
                <a href="<?= url('pages/profile.php') ?>?view=reviews">View All</a>
            </div>
            ...
            <?php foreach (array_slice($myReviews, 0, $view === 'reviews' ? 100 : 3) as $r): ?>
                <article class="review">
                    <div class="review__head">
                        ...
                        <b><a href="<?= url('pages/place.php') ?>?id=<?= (int)$r['place_id'] ?>"><?= e($r['name']) ?></a></b>
        ...
        <?php if ($view === 'settings'): ?>
            ...
            <form method="post" action="<?= url('pages/profile.php') ?>?view=settings" class="card card__pad" style="max-width:520px;">
