...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancel_id'])) {
    ...
    redirect('pages/my-bookings.php?tab=cancelled');
    exit;
}
...
<div class="container page">
    ...
    <nav class="tabs">
        <?php foreach ($valid as $t): ?>
            <a href="<?= url('pages/my-bookings.php') ?>?tab=<?= $t ?>" class="<?= $tab === $t ? 'is-active' : '' ?>">
                <?= ucfirst($t) ?> (<?= $counts[$t] ?>)
            </a>
        <?php endforeach; ?>
    </nav>
    ...
    <?php if (!$bookings): ?>
        <div class="card card__pad">
            ...
            <a class="btn btn--sm" href="<?= url('pages/explore.php') ?>">Find a study space</a>
        </div>
    <?php endif; ?>
    ...
    <?php foreach ($bookings as $b): ?>
        <article class="booking-row">
            ...
            <div class="booking-row__side">
                ...
                <a class="btn btn--outline btn--sm" href="<?= url('pages/place.php') ?>?id=<?= (int)$b['place_id'] ?>">View Details</a>
                <?php if ($b['status'] === 'upcoming'): ?>
                    <form method="post" action="<?= url('pages/my-bookings.php') ?>" onsubmit="return confirm('Cancel this booking?');">
                        <input type="hidden" name="cancel_id" value="<?= (int)$b['id'] ?>">
                        <button class="btn btn--ghost btn--sm" type="submit">Cancel</button>
                    </form>
                <?php elseif ($b['status'] === 'completed'): ?>
                    <a class="btn btn--ghost btn--sm" href="<?= url('pages/review.php') ?>?id=<?= (int)$b['place_id'] ?>">Write a review</a>
                <?php endif; ?>
            </div>
        </article>
    <?php endforeach; ?>
